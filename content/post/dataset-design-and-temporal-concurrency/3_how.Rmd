...
E <- local({
  ...
  mbr_events <- replicate(
    length(mbrs), {
      .out <- sample(.max_events, 1);
      .out <- replicate(
        .out
        , c(sample(1:20, 1)
            , sample(30:60, 1, prob = runif(n = length(30:60), 0.1, 0.5))
            )
        ) |> 
        as.vector() |> 
        sort() |> 
...
The fun part is taking into account $h_1$, a time-based requirement that adds a layer of complexity to the report matrix depending on the metric. For example, consider [ $\gamma_4$]{msg_id=g4}, an, easy metric to aggregate across each of $h_k$. Contrast that with [ $\gamma_1$]{msg_id=g1}, which presents a temporal **windowing** problem with respect to [ $h_1$]{msg_id=h1} since both involve windows of time:
